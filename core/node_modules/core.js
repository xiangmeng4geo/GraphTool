!function(){
	...
	conf.User = {
		...
		get: function(name){
			var conf_user_path = path.join(path_conf_project, name+SUFFIX_JSON);
	...
	!function(){
		...
		function getNewest(dir, rule){
			...
			console.log(rule, time_start, time_end);
		...
		function _getMicapsData(file_path, callback){
			var cache_file = path.join(file_util.path.tmp_micaps, encrypt(file_path));
			if(fs.existsSync(cache_file) && fs.statSync(cache_file).mtime > fs.statSync(file_path).mtime){
				...
				parse_micaps(file_path, function(err, data){
					if(err){
					...
					if(data){
						fs.writeFile(cache_file, JSON.stringify(data)); //异步保存数据
					}
				});
			}
		}
