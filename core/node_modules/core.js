!function(){
	...
	var conrec = require('./micaps_parser/utils/conrec').conrec;
	...
	var encrypt = function (str,private_key){
	    if(str && str.toString){
	        return crypto.createHash('sha1').update(str.toString()+(private_key||DEFAULT_PRIVATE_KEY)).digest('hex');
	    }
	    return '';
	}
	/*可逆加密解密 http://blog.fens.me/nodejs-crypto/*/
	var METHOD_ALGORITHM = 'aes-256-cbc',
		DEFAULT_KEY = 'GraphTool'+DEFAULT_PRIVATE_KEY;
	...
	!function(){
		...
		var file_path = {
			...
			tmp_logs: path.join(_path_tmp, 'logs'),
			tmp_img: path.join(_path_tmp, 'img'),
	...
	var CONF_FILE_NAME_TASKS = '_tasks_';
	conf.get = getConf;
	...
	conf.User = {
		...
		getTasks: function(){
			var conf_user_path = _getConfPath(CONF_FILE_NAME_TASKS);
			return getConfJson(conf_user_path);
		},
		setTasks: function(data){
			this.write(CONF_FILE_NAME_TASKS, data);
		},
		get: function(name){
	...
	!function(){
		...
		function _getMicapsData(file_path, option, callback){
			if(option){
				var grid_space = option.grid_space;
				if(grid_space){
					var interpolate = _const.interpolate;
					var list = interpolate.l;
					for(var i = 0, j = list.length; i<j; i++){
						if(list[i].v == grid_space){
							break;
						}
					}
					if(i == j){
						option.grid_space = interpolate.d;
					}
				}
			}
		...
		var _raster2vector = function(data, color_novalue, params, blendent){
			var cache_file_micaps = (params? params.path: null) || null;
			//多条件缓存，防止由于micaps缓存路径或配色或novalue_color更改时缓存命中错误
			var cache_file = cache_file_micaps ? path.join(file_util.path.tmp_micaps, encrypt('raster2vector_'+color_novalue+'_'+cache_file_micaps+JSON.stringify(blendent))): null;
		...
		var _getPolygons = function(data, color_novalue, params, blendent){
			var cache_file_micaps = (params? params.path: null) || null;
			//多条件缓存，防止由于micaps缓存路径或配色或novalue_color更改时缓存命中错误
			var cache_file = cache_file_micaps ? path.join(file_util.path.tmp_micaps, encrypt('getPolygons_'+color_novalue+'_'+cache_file_micaps+JSON.stringify(blendent))): null;
			...
			if(cache_file && cache_file_micaps && 
				...
				var data = conrec(data, blendent, color_novalue);
				if(cache_file){
					fs.writeFileSync(cache_file, JSON.stringify(data)); //异步保存数据
				}
				return data;
		...
		function _getTmpAutowork(){
			return path.join(file_util.path.tmp, encrypt('autowork'));
		}
		function _getTmpMain(){
			return path.join(file_util.path.tmp, encrypt('main'));
		}
		file_util.tmp = {
			legend: {
			...
			clear: function(){
				file_util.rm(file_util.path.tmp_legend, true);
				file_util.rm(file_util.path.tmp_micaps, true);
			},
			autowork: {
				get: function(){
					return file_util.readFile(_getTmpAutowork(), true);
				},
				set: function(data){
					fs.writeFileSync(_getTmpAutowork(), JSON.stringify(data));
				}
			},
			main: {
				get: function(){
					return file_util.readFile(_getTmpMain(), true);
				},
				set: function(data){
					fs.writeFileSync(_getTmpMain(), JSON.stringify(data));
				}
			}
		}
	}();
	;(function(){
		...
		var _addTime = function(args){
			args = [].slice.call(args);
			args.unshift(new Date().format());
			return args;
		}
		var logPath = util_file_path.tmp_logs;
		function Logger(prefix, getLogFileName){
			prefix = prefix||'';
			getLogFileName || (getLogFileName = function(msgDate, prefix_name){
				return path.join(logPath, prefix_name+'_'+(msgDate.format('yyyy-MM-dd'))+'.log');
			});
			// logPath = path.normalize(path.join(logPath, '.'));
			var cacheName = getLogFileName(new Date(), prefix);//根据logPath和getLogFileName来确定缓存名称
			if(fnCache[cacheName]){
				return fnCache[cacheName];
			}
			util_file.mkdir(logPath);
			...
			var deal = function(){
				...
				tempData.forEach(function(info){
					var t = getLogFileName(info[0], prefix);
					tempObj[t] || (tempObj[t] = []);
	...
	exports.util = util;
	exports.conf = conf;
	exports.raster2vector_test = function(a){
		raster2vector = a;
	}
}();