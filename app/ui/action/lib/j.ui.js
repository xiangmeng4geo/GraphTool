...
(function(factory) {
		...
		$.position = {
			...
			getWithinInfo: function(element) {
				...
				return {
					...
					width: isWindow || isDocument ? withinElement.width() : withinElement
						.outerWidth(),
					height: isWindow || isDocument ? withinElement.height() : withinElement
						.outerHeight()
				};
			}
		};
	...
	var accordion = $.widget("ui.accordion", {
		...
		refresh: function() {
			...
			if ((options.active === false && options.collapsible === true) || !this
				.headers.length) {
	...
	var menu = $.widget("ui.menu", {
		...
		_create: function() {
			...
			this._on({
				...
				focus: function(event, keepActiveItem) {
					...
					var item = this.active || this.element.find(this.options.items).eq(
						0);
		...
		nextPage: function(event) {
			...
			if (this._hasScroll()) {
				...
				this.focus(event, this.activeMenu.find(this.options.items)[!this.active ?
					"first" : "last"]());
			}
		},
			...
			if (this.type === "checkbox") {
				...
				this.buttonElement
					...
					.bind("keydown" + this.eventNamespace, function(event) {
						...
						if (event.keyCode === $.ui.keyCode.SPACE || event.keyCode === $.ui.keyCode
							.ENTER) {
							$(this).addClass("ui-state-active");
						}
					})
	...
	$.extend(Datepicker.prototype, {
		...
		_optionDatepicker: function(target, name, value) {
			...
			if (inst) {
				...
				if (minDate !== null && settings.dateFormat !== undefined && settings.minDate ===
					undefined) {
					inst.settings.minDate = this._formatDate(inst, minDate);
				}
				if (maxDate !== null && settings.dateFormat !== undefined && settings.maxDate ===
					undefined) {
		...
		_doKeyPress: function(event) {
			...
			if ($.datepicker._get(inst, "constrainInput")) {
				...
				chr = String.fromCharCode(event.charCode == null ? event.keyCode :
					event.charCode);
				return event.ctrlKey || event.metaKey || (chr < " " || !chars || chars
					.indexOf(chr) > -1);
			}
		},
		...
		parseDate: function(format, value, settings) {
			...
			var iFormat, dim, extra,
				...
				dayNamesShort = (settings ? settings.dayNamesShort : null) || this._defaults
				.dayNamesShort,
				dayNames = (settings ? settings.dayNames : null) || this._defaults.dayNames,
				monthNamesShort = (settings ? settings.monthNamesShort : null) || this
				._defaults.monthNamesShort,
				monthNames = (settings ? settings.monthNames : null) || this._defaults
				.monthNames,
				year = -1,
				month = -1,
				day = -1,
		...
		_generateHTML: function(inst) {
			var maxDraw, prevText, prev, nextText, next, currentText, gotoDate,
				...
				currentDate = this._daylightSavingAdjust((!inst.currentDay ? new Date(
						9999, 9, 9) :
			...
			for (row = 0; row < numMonths[0]; row++) {
				...
				for (col = 0; col < numMonths[1]; col++) {
					...
					printDate = this._daylightSavingAdjust(new Date(drawYear, drawMonth,
						1 - leadDays));
						...
						for (dow = 0; dow < 7; dow++) { // create date picker days
							...
							tbody += "<td class='" +
								...
								((!otherMonth || showOtherMonths) && daySettings[2] ? " title='" +
									daySettings[2].replace(/'/g, "&#39;") + "'" : "") + // cell title
								(unselectable ? "" :
			...
			if (!inst.yearshtml) {
				...
				if (secondary || !changeYear) {
					...
					determineYear = function(value) {
						var year = (value.match(/c[+\-].*/) ? drawYear + parseInt(value.substring(
								1), 10) :
		...
		_adjustInstDate: function(inst, offset, period) {
			var year = inst.drawYear + (period === "Y" ? offset : 0),
				...
				(period === "D" ? offset : 0),
				date = this._restrictMinMax(inst, this._daylightSavingAdjust(new Date(
					year, month, day)));
	...
	$.widget("ui.draggable", $.ui.mouse, {
		...
		_mouseStart: function(event) {
			...
			var is_rotate = this.is_rotate = this.helper.css('transform') !==
				'none'; //暂时不考虑scale及translate
			if (is_rotate) {
		...
		_isRootNode: function(element) {
			return (/(html|body)/i).test(element.tagName) || element === this.document[
				0];
		},
		...
		_setContainment: function() {
			...
			if (o.containment === "window") {
				this.containment = [
					...
					this.margins.left,
					$(window).scrollTop() + ($(window).height() || document.body.parentNode
			...
			this.containment = [
				(parseInt(c.css("borderLeftWidth"), 10) || 0) + (parseInt(c.css(
					"paddingLeft"), 10) || 0), (parseInt(c.css("borderTopWidth"), 10) ||
					0) + (parseInt(c.css("paddingTop"), 10) || 0), (isUserScrollable ?
					Math.max(ce.scrollWidth, ce.offsetWidth) : ce.offsetWidth) -
				(parseInt(c.css("borderRightWidth"), 10) || 0) -
				(parseInt(c.css("paddingRight"), 10) || 0) -
				this.helperProportions.width -
				this.margins.left -
				this.margins.right, (isUserScrollable ? Math.max(ce.scrollHeight, ce.offsetHeight) :
					ce.offsetHeight) -
			...
			if (constrainPosition) {
				...
				if (o.grid) {
					...
					pageY = containment ? ((top - this.offset.click.top >= containment[1] ||
						top - this.offset.click.top > containment[3]) ? top : ((top - this
						.offset.click.top >= containment[1]) ? top - o.grid[1] : top + o.grid[
						1])) : top;

					left = o.grid[0] ? this.originalPageX + Math.round((pageX - this.originalPageX) /
						o.grid[0]) * o.grid[0] : this.originalPageX;
					pageX = containment ? ((left - this.offset.click.left >= containment[
						0] || left - this.offset.click.left > containment[2]) ? left : ((
						left - this.offset.click.left >= containment[0]) ? left - o.grid[
	...
	$.ui.plugin.add("draggable", "scroll", {
		start: function(event, ui, i) {
			...
			if (i.scrollParentNotHidden[0] !== i.document[0] && i.scrollParentNotHidden[
					0].tagName !== "HTML") {
		...
		start: function(event, ui, instance) {
			var min,
				...
				group = $.makeArray($(o.stack)).sort(function(a, b) {
					return (parseInt($(a).css("zIndex"), 10) || 0) - (parseInt($(b).css(
						"zIndex"), 10) || 0);
				});
	...
	$.widget("ui.rotatable", $.ui.mouse, {
		...
		_create: function() {
			...
			var $handle = this.handle = $('<div></div>').addClass(
				'ui-rotatable-handle');
		...
		_mouseDrag: function(e) {
			...
			if (origin) {
				var angle = Math.atan2(e.pageY - origin.y, e.pageX - origin.x) + Math.PI /
					2;
				if (angle < 0) {
					angle += Math.PI * 2;
				}
				angle = angle / Math.PI * 180;
				_this.element.data('angle', angle).css('transform', 'rotate(' + angle +
					'deg)');
			...
			this._on(this.uiDialog, {
				keydown: function(event) {
					...
					if ((event.target === last[0] || event.target === this.uiDialog[0]) &&
						...
						event.preventDefault();
					} else if ((event.target === first[0] || event.target === this.uiDialog[
							0]) && event.shiftKey) {
						this._delay(function() {
							last.focus();
						});
	...
	*/
				...
				function() {
					...
					var parentInstance, scope, parent,
						intersects = $.ui.intersect(draggable, this, this.options.tolerance,
							event),
						c = !intersects && this.isover ? "isout" : (intersects && !this.isover ?
							"isover" : null);
					if (!c) {
						return;
					}
	...
	(function() {
		...
		$.extend(baseEasings, {
			...
			Elastic: function(p) {
				return p === 0 || p === 1 ? p :
					-Math.pow(2, 8 * (p - 1)) * Math.sin(((p - 1) * 80 - 7.5) * Math.PI /
						15);
			},
			Back: function(p) {
				return p * p * (3 * p - 2);
	...
	var selectable = $.widget("ui.selectable", $.ui.mouse, {
		...
		_mouseStart: function(event) {
			...
			$(event.target).parents().addBack().each(function() {
				...
				if (selectee) {
					doSelect = (!event.metaKey && !event.ctrlKey) || !selectee.$element
		...
		_mouseDrag: function(event) {
			...
			this.selectees.each(function() {
				...
				if (options.tolerance === "touch") {
					...
					hit = (selectee.left > x1 && selectee.right < x2 && selectee.top >
	...
	var sortable = $.widget("ui.sortable", $.ui.mouse, {
		...
		_isFloating: function(item) {
			return (/left|right/).test(item.css("float")) || (/inline|table-cell/).test(
				item.css("display"));
		},
		...
		_create: function() {
			...
			this.floating = this.items.length ? o.axis === "x" || this._isFloating(
		...
		cancel: function() {
			...
			if (this.placeholder) {
				...
				if (this.options.helper !== "original" && this.helper && this.helper[0]
					.parentNode) {
					this.helper.remove();
				}
		...
		_intersectsWithPointer: function(item) {
			...
			return this.floating ?
				(((horizontalDirection && horizontalDirection === "right") ||
					verticalDirection === "down") ? 2 : 1) : (verticalDirection && (
		...
		_intersectsWithSides: function(item) {
			...
			var isOverBottomHalf = this._isOverAxis(this.positionAbs.top + this.offset
					.click.top, item.top + (item.height / 2), item.height),
				isOverRightHalf = this._isOverAxis(this.positionAbs.left + this.offset
				...
				for (i = connectWith.length - 1; i >= 0; i--) {
					...
					for (j = cur.length - 1; j >= 0; j--) {
						...
						if (inst && inst !== this && !inst.options.disabled) {
							queries.push([$.isFunction(inst.options.items) ? inst.options.items
								.call(inst.element) : $(inst.options.items, inst.element).not(
			...
			if (!o.placeholder || o.placeholder.constructor === String) {
				...
				o.placeholder = {
					...
					update: function(container, p) {
						...
						if (!p.height()) {
							p.height(that.currentItem.innerHeight() - parseInt(that.currentItem
								.css("paddingTop") || 0, 10) - parseInt(that.currentItem.css(
								"paddingBottom") || 0, 10));
						}
						if (!p.width()) {
							p.width(that.currentItem.innerWidth() - parseInt(that.currentItem
								.css("paddingLeft") || 0, 10) - parseInt(that.currentItem.css(
								"paddingRight") || 0, 10));
						}
					}
				};
		...
		_contactContainers: function(event) {
			...
			if (this.containers.length === 1) {
				...
				for (j = this.items.length - 1; j >= 0; j--) {
					if (!$.contains(this.containers[innermostIndex].element[0], this.items[
							j].item[0])) {
						continue;
					}
				...
				itemWithLeastDistance ? this._rearrange(event, itemWithLeastDistance,
					null, true) : this._rearrange(event, null, this.containers[
					innermostIndex].element, true);
				this._trigger("change", event, this._uiHash());
		...
		_getRelativeOffset: function() {
			...
			if (this.cssPosition === "relative") {
				...
				return {
					top: p.top - (parseInt(this.helper.css("top"), 10) || 0) + this.scrollParent
						.scrollTop(),
					left: p.left - (parseInt(this.helper.css("left"), 10) || 0) + this.scrollParent
						.scrollLeft()
				};
			} else {
				return {
		...
		_setContainment: function() {
			...
			if (o.containment === "document" || o.containment === "window") {
				this.containment = [
					...
					$(o.containment === "document" ? document : window).width() - this.helperProportions
					.width - this.margins.left, ($(o.containment === "document" ?
			...
			if (!(/^(document|window|parent)$/).test(o.containment)) {
				...
				this.containment = [
					...
					(parseInt($(ce).css("borderLeftWidth"), 10) || 0) - (parseInt($(ce).css(
						"paddingRight"), 10) || 0) - this.helperProportions.width - this.margins
					.left,
					co.top + (over ? Math.max(ce.scrollHeight, ce.offsetHeight) : ce.offsetHeight) -
					(parseInt($(ce).css("borderTopWidth"), 10) || 0) - (parseInt($(ce).css(
			...
			if (this.originalPosition) { //If we are not dragging yet, we won't check for options
				...
				if (o.grid) {
					top = this.originalPageY + Math.round((pageY - this.originalPageY) /
						o.grid[1]) * o.grid[1];
					pageY = this.containment ? ((top - this.offset.click.top >= this.containment[
							1] && top - this.offset.click.top <= this.containment[3]) ? top :
						((top - this.offset.click.top >= this.containment[1]) ? top - o.grid[
							1] : top + o.grid[1])) : top;

					left = this.originalPageX + Math.round((pageX - this.originalPageX) /
						o.grid[0]) * o.grid[0];
					pageX = this.containment ? ((left - this.offset.click.left >= this.containment[
							0] && left - this.offset.click.left <= this.containment[2]) ? left :
						((left - this.offset.click.left >= this.containment[0]) ? left - o.grid[
							0] : left + o.grid[0])) : left;
				}
