...
function reback(data, file_path, save_path) {
	...
	_getChinaBoundaries(data_new, save_path);
}

function _getChinaBoundaries(data, save_path) {
	var SCALE = 10000;

	function _numFormat(num, digit) {
		return parseFloat(num.toFixed(digit || 4));
	}
	...
	for (var i = 0, j = features.length; i < j; i++) {
		...
		if (geometry.type == 'Polygon') {
			clipItems.push(_getCoordinates(coordinates));
		} else {
			for (var i_c = 0, j_c = coordinates.length; i_c < j_c; i_c++) {
				clipItems.push(_getCoordinates(coordinates[i_c]));
			}
		}
	}
	var ClipperLib = require('../../core/node_modules/micaps_parser/utils/clipper');
	var TYPE_FILL = ClipperLib.PolyFillType.pftNonZero;
	var cpr = new ClipperLib.Clipper();
	ClipperLib.JS.ScaleUpPaths(clipItems, SCALE);
	cpr.AddPaths(clipItems, ClipperLib.PolyType.ptSubject, true);
	...
	for(var i = 0, j = solution_paths.length; i<j; i++){
		var items = solution_paths[i];
		var items_new = [];
		for(var i_i = 0, j_i = items.length; i_i < j_i; i_i++){
			var item = items[i_i];
			items_new.push([
				item.X /SCALE,
				item.Y/SCALE
		...
		coordinates_new.push(items_new);
	}
	geo_data.features = [{
		"type": "Feature",
		"geometry": {
	...
	var save_path_china_b = save_path.replace('.json', '_b.json');
	fs.writeFileSync(save_path_china_b, JSON.stringify(geo_data));
	console.log('write file', save_path_china_b);
}
var TYPE_NATION = 1,
...
function format(file_path, save_path) {
	save_path = path.resolve(__dirname, save_path);
